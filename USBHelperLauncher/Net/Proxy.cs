...
namespace USBHelperLauncher.Net
{
    class Proxy : IDisposable
    {
        ...
        private StreamWriter verboseLog;
        ...
        {
            if (Program.Verbose && oS.oFlags["x-Fiddler-Generated"] != null)
            {
                if (verboseLog == null)
                {
                    verboseLog = new StreamWriter(Path.Combine(Program.GetLauncherPath(), "verbose.log"), false);
                    verboseLog.AutoFlush = true;
                    verboseLog.WriteLine("Generated by USBHelperLauncher " + Program.GetVersion());
                }
                var sb = new StringBuilder();
                sb.Append('-', 20).AppendLine();
                sb.AppendLine("Session Length: " + (DateTime.UtcNow - Program.GetSessionStart()).ToString(@"hh\:mm\:ss"));
                sb.AppendLine(oS.RequestHeaders.ToString(true, false, true));
                sb.AppendLine(oS.ResponseHeaders.ToString(true, false));
                string body = Encoding.ASCII.GetString(oS.responseBodyBytes);
                if (body.Any(c => char.IsControl(c) && c != '\r' && c != '\n'))
                {
                    sb.AppendLine("Response body contains binary data.");
                }
                else
                {
                    sb.AppendLine(body);
                }
                verboseLog.Write(sb.ToString());
            }
